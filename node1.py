...
def check_promise(server_conn, message,extracted_info, ballot_compare):
    global time_promise, count_promise
    while time_promise != "":
        time.sleep(0.1)
        if time_promise == "":
            continue
...
def check_accepted_timeout(server_conn):
    global pending_accepted, queue, leader_name, semaphore
...
def get_message_from_server(server_conn):
    global shutdown, leader_name, time_promise, time_accepted, count_accepted, count_promise, pending_promise, is_leader
    buffer = ""
    ballot_compare = []
    extracted_info = []
    while not shutdown:
...
def update_context(message):
    ...
    if int_ballot[2] > ballot[2]:
        ...
        semaphore += 1
        if leader_name != "node1":
            is_leader = False
def receive_messages(server_conn, message, extracted_info, ballot_compare):
    global message_queue, acceptVal, acceptNum, ballot, pending_ack, processed_messages, context, leader_name, count_promise, count_accepted, pending_accepted, is_leader, pending_promise, semaphore, decide_timeout
    if "-" in message:
...
def process_decide_message(message, server_conn):
    global ballot, semaphore, is_leader
    ballot[2] += 1
...
def processBallotMessage(message, node_name, server_conn):
    global ballot
    global leader_name, semaphore, is_leader
    ...
    print(f"----PHASE 1b: Promise Phase-----")
...
def processAccept(message, node_name, server_conn):
    ...
    if ballot == leader_ballot:
        ...
        print(f"Did not accept ballot: {leader_ballot} my ballot: {ballot}")
...
def start_node():
    ...
    port_server_connection.send("node1".encode('utf-8'))
