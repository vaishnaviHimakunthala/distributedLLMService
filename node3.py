...
def check_promise(server_conn, message,extracted_info, ballot_compare):
    global time_promise, count_promise, is_leader, semaphore
    while time_promise != "":
        time.sleep(0.1)
        if time_promise == "":
            continue
...
def check_accepted_timeout(server_conn):
    global pending_accepted, queue, semaphore, is_leader
...
def get_message_from_server(server_conn):
    global shutdown, time_accepted, time_promise, leader_name, count_promise, count_accepted, pending_promise, is_leader
    buffer = ""
    extracted_info = []
    ballot_compare = []
    while not shutdown:
...
def update_context(message):
    ...
    if int_ballot[2] > ballot[2]:
        ...
        semaphore = 1
        if leader_name != "node3":
            is_leader = False



def receive_messages(server_conn, message, extracted_info, ballot_compare):
    global message_queue, acceptVal, acceptNum, ballot, pending_ack, pending_promise, context, processed_messages, leader_name, count_accepted, count_promise, semaphore, pending_accepted, is_leader, decide_timeout
    ...
    if "forward: " in message:
        ...
        if "+FAIL" in message:
            actual_message = message.split("+FAIL")[1]
            extracted_info_1 = extractPromiseMessage(actual_message)
            if extracted_info_1[3] != "":
                acceptVal[ballot[2]] = extracted_info_1[3]
                acceptNum[ballot[2]] = extracted_info_1[1]
                original = extracted_info_1[2]
            else:
                acceptVal[ballot[2]] = extracted_info_1[2]
        ...
        handle = threading.Thread(target=processAccept, args=(message, original_node_name,server_conn))
        handle.start()
        ...
        if "+FAIL" in message:
            actual_message = message.split("+FAIL")[1]
            info = actual_message.split("accepted: ")[1].split(", ")
            command = info[3]
...
def process_decide_message(message, server_conn):
    ...
    if "query" in message:
        ...
        message_queue.put(f"node2-Decided: id: {contextid} {answer}")
...
def send_decide_messages(message, server_conn):
    ...
    message_queue.put(f"node2-Decide: {message}")
...
def sendAccept(message, server_conn):
    ...
    message_queue.put(f"node2-accept: {ballot[0]}, {ballot[1]}, {ballot[2]}, {message}")
...
def startElection(message, server_conn):
    ...
    ballot = [ballot[0]+1, 3, ballot[2]]
    print("----PHASE 1a: Prepare phase----")
    message_queue.put(f"node1-prepare: {ballot[0]}, {ballot[1]}, {ballot[2]}, {message}")
    message_queue.put(f"node2-prepare: {ballot[0]}, {ballot[1]}, {ballot[2]}, {message}")
...
def start_node():
    ...
    port_server_connection.send("node3".encode('utf-8'))
